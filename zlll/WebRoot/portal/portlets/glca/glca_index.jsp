...
<script>
function add(){
	window.location.href("<%=request.getContextPath() %>/portal/glca/add.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>");
}
function query(){
	...
	if(hasChecked()){
       if(tmain.getSelectedRow().length>1){
          alert("请选择一条进行修改!");
             ...
             $('mainListForm').action="<%=request.getContextPath() %>/portal/glca/mod.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>";
             $('mainListForm').submit();
       }
     }else{
       alert("请选择要修改的数据!");
...
//debugger;
	if(hasChecked()){
        if(confirm('确定要注销掉该用户的CA吗?')) {
          ...
          $('mainListForm').action="<%=request.getContextPath() %>/portal/glca/del.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>";
          $('mainListForm').submit();    
	    }
     }else{
        alert("请选择要注销的用户!");
...
<form id="queryform"
	action="<%=request.getContextPath()%>/portal/glca/index.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>"
	method="post">
