...
function add(){
	window.location.href("<%=request.getContextPath() %>/portal/userInforCollection/add.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>");
}
function mod(){
	if(hasChecked()){
       if(tmain.getSelectedRow().length>1){
          alert("请选择一条用户信息进行修改!");
             ...
             $('mainListForm').action="<%=request.getContextPath() %>/portal/userInforCollection/mod.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>";
             $('mainListForm').submit();
       }
     }else{
       alert("请选择要修改的数据!");
...
function del(){
	if(hasChecked()){
        if(confirm('确定要注销所选用户信息吗?')) {
          ...
          $('mainListForm').action="<%=request.getContextPath() %>/portal/userInforCollection/del.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>";
          $('mainListForm').submit();    
	    }
     }else{
        alert("请选择要删除的用户信息!");
   ...
   with (event){   
     ...
     if (keyCode==116 || (ctrlKey && keyCode==82)){  
     	if(d_url.indexOf('common/post/index')<=0){
			window.location='<%=request.getContextPath() %>/portal/userInforCollection/index.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>';
		} 
       event.keyCode = 0;   
...
<form id="queryform"
	action="<%=request.getContextPath()%>/portal/userInforCollection/index.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>"
	method="post">
