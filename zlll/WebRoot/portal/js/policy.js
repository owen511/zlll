...
Ext.lt.portal.component.policy = new function () {
	this.getPolicy = function (postinfo,server) {
		...
		if(null == postlist || postlist.size()<1){
			...
			for(var i=0;i<postlist.size();i++){
				...
				setHtml.push("["+titlename+"] <a class='gg' href=\"javascript:Ext.lt.portal.component.policy.preview('"+postlist[i].id+"','"+postlist[i].fileid+"')\">"+postlist[i].posttitle+"</a>");
		...
		setHtml.push("</table>");
	    setHtml.push("</div>");
		var retmenupanel = new Ext.Panel({title:titlename,panelheight:divHight,height:panelheight, html:setHtml.join('')});
	...
	this.preview=function(id,postfileid){
    	...
    	var url = _ROOT_PATH_+"/portal/portal2/login/hlj_preview.jsp?id="+id+"&type="+3;
