...
<form name="form1" id="form1"
	action="<%=request.getContextPath()%>/portal/userinfo/save.do"
	method="post">
	...
	<div id="form_table">
		<table cellpadding="0" cellspacing="0">
			<tr>
				...
				<td nowrap="nowrap">
					<input type="text" id="code" maxlength="50" name="code" onchange="checkUserCode()" />
				</td>
				<th nowrap="nowrap">
					用户名称
					<span>*</span>
			...
			<tr>
				...
				<td nowrap="nowrap">
					<input type="radio" name="logotype"  value = "0" >密码登录</input>
					<input type="radio" name="logotype"  value = "1" >CA登录</input>
				</td>
				<th nowrap="nowrap">
					用户密码
					<span>*</span>
			...
			<tr>
				...
				<td nowrap="nowrap">
					<input type="text"
						value="<c:out value='${managerUserDTO.code}'/>-<c:out value='${managerUserDTO.name}'/>"
						disabled="disabled" name=manageridStr id=manageridStr />
					<input name=managerid id=managerid type="hidden"
						value="<c:out value='${managerUserDTO.userid}'/>" />
				</td>
			</tr>
			<tr>
				<th nowrap="nowrap">
				...
				<td nowrap="nowrap">
				   <c:choose>
				       ...
				       <c:otherwise>
							<input type="text" name=userclsidStr id=userclsidStr
								readonly="readonly" onclick="selectUserCls()">
							<input type="hidden" name=userclsid id=userclsid>
				...
				<td nowrap="nowrap">
					<input name="overduedate" type="text" id="overduedate" value=""
	...
	<div>
		...
		<div id="confirm_exit_btn" style='width:100%;display:block;'>
			...
			<input name="submit2" type="button" class="button_style"
				onmouseover="this.className='OverBtn'"
				onmouseout="this.className='button_style'"
				onmousedown="this.className='down'" value="保存并退出" onclick="saveExit()" />
	...
	function saveContinue(){
	    ...
	    if(checkInput(formObject)){
		    ...
		    formObject.action = "<%=request.getContextPath()%>/portal/userinfo/saveContinue.do";
		    formObject.submit();
	    }
	}
	...
	function saveExit(){
	    ...
	    if(checkInput(formObject)){
		    formObject.selectedRoleJson.value = selectedRole.data.toJSON();
		    formObject.action = "<%=request.getContextPath()%>/portal/userinfo/save.do";
		    formObject.submit();
	    }
	}
...
String.prototype.getBytes = function() {       
	...
	function checkInput(formObject){
		...
		if(formObject.code.value.trim().getBytes() > 50){
			alert("编码字符长度过长");
			return false;
		}
		...
		if(formObject.name.value.trim().getBytes > 50){
			alert("用户名称字符长度过长");
			return false;
		}
		if(checkRadio() == false){
		...
		if(formObject.overduedate.value != ""){
			if(formObject.overduedate.value < nowDate){
		       alert("过期时间不能小于当前日期！");
		       return false;
		    }
		}
	...
	function init(){
		...
		if(code != ''){
			formObject.code.value = code;
		}
		var name = '<c:out value="${name}"/>';
		if(name != ''){
			formObject.name.value = name;
		}
		var type = '<c:out value="${type}"/>';
		if(type != ''){
			formObject.type.value = type;
		}
		var organtype = '<c:out value="${organtype}"/>';
		if(organtype != ''){
			formObject.organtype.value = organtype;
		}
		var organidStr = '<c:out value="${userViewDTO.organidStr}"/>';
		if(organidStr != ''){
			formObject.organidStr.value = organidStr;
		}
		var organid = '<c:out value="${userViewDTO.organid}"/>';
		if(organid != ''){
			formObject.organid.value = organid;
		}
		var organCode = '<c:out value="${userViewDTO.organCode}"/>';
		if(organCode != ''){
			formObject.organCode.value = organCode;
		}
		var userclsidStr = '<c:out value="${userViewDTO.userclsidStr}"/>';
		if(userclsidStr != ''){
			formObject.userclsidStr.value = userclsidStr;
		}
		var userclsid = '<c:out value="${userViewDTO.userclsid}"/>';
		if(userclsid != ''){
			formObject.userclsid.value = userclsid;
		}
		var overduedate = '<c:out value="${overduedate}"/>';
		if(overduedate != ''){
			formObject.overduedate.value = overduedate;
		}
		var state = '<c:out value="${state}"/>';
		if(state != ''){
			formObject.state.value = state;
		}else{
			formObject.state.value = "1";
		}
		var remark = '<c:out value="${remark}"/>';
		if(remark != ''&& remark!=null && remark!='null' ){
			formObject.remark.value = remark;
		}
	}
	init();
	function checkRadio(){
