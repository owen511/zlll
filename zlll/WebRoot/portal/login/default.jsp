				   ...
				   for(i=0; i<pendingtasks.size();i++) {
				      ...
				      if(pendingtask.count>0){
							   ...
							   { 
							      ...
							      if(detail.menuname == name_tem[o] ){
							         ...
							         {  
							         	...
							         	taskStr=taskStr+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+"<a href=\""+detail.JobUrl+"\" target='_blank'><font color = '"+detail.color+"'>"+detail.status+"<"+detail.totalcount+"条></font></a>";
							         	taskStr=taskStr+"<br/>";  
			                         }
			                         else
			                         {
				       ...
				       for(j=0;j<details.size();j++)
					   {
				         ...
				         {  
				            if(detail.totalcount>0){
					         	taskStr=taskStr+"<a href=\""+detail.JobUrl+"\" target='_blank'><font color = '"+detail.color+"'>"+detail.status+"<"+detail.totalcount+"条></a>";
				         	}
			...
			<%  //if(report_list.size()>0){%>
			 <div id="reportShow" style="display:block;">
						...
						<% for(int o=0;o<report_list.size();o++){
							if(o==0){
							%><li class="selected" onclick="switchReport(this,<%=o+1 %>,<%=report_list.size() %>)">图表<%=o+1 %></li><%
							}else{
							%><li class="noselected" onclick="switchReport(this,<%=o+1 %>,<%=report_list.size() %>)">图表<%=o+1 %></li><%
							}
						}%>
						</ul>
						</div>
