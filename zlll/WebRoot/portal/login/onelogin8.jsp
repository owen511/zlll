...
<style>
  ...
  input{padding:0;margin:0;height:15px; width:65px;font-size:12px;line-height:15px;}
  #login_button {
	border: 0px;
	text-align: center;
	padding-top: 3px;
...
<div style="text-align:center;display:block;height:74px; width:240px; font-size:12px; border:3px #ccc double; background:url(<%=request.getContextPath()%>/images/login/small_login.gif); background-repeat:repeat; background-position:left top;">
<p style="margin-bottom:2px;font-size:12px;line-height:24px;color:#A50021; font-weight:bold; text-align:left; border-bottom:1px #CCC solid;background:url(<%=request.getContextPath()%>/images/login/login_title.gif); background-repeat:repeat; background-position:left top;"><a style="height:20px; line-height:20px; background:url(<%=request.getContextPath()%>/images/login/login_title_dot.gif); background-repeat:no-repeat; background-position:center center; ">&nbsp;&nbsp;&nbsp;&nbsp;</a>财政一体化管理信息系统</p>
  <form id="form1" name="form1" method="post" target="_blank" action="<%=request.getContextPath()%>/login.do">
    <table border="0" cellspacing="0" cellpadding="0" style ="width:97%; display:block; height:47px;">
      <tr>
      <!-- -请不要添加默认的用户和密码  -->
      <% String isportalca = (String)session.getAttribute("isportalca");
	    ...
	    %>
        <td width="8%" align="right" nowrap="nowrap"><font color="#000">o8用户：</font></td>
        <td width="25%" nowrap="nowrap" colspan="3" align="left"><input type="text" name="username" value=""/></td>
        <td width="10%" align="right" nowrap="nowrap" style="top_color"><font color="#000">密码：</font></td>
        <td width="25%" nowrap="nowrap" colspan="3" align="left"><input type="password" name="password" value=""/></td>
        </tr>
        <tr>
        <% //}%>
        <!-- 宁夏集中登录地区选择 -->
      ...
      <%if(Globals.isMultiDataSourceDeployMode()){ %>
        <td width="23%" nowrap="nowrap" align="left">
