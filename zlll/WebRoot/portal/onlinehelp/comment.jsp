...
<html>
  ...
  <script type="text/javascript" src="<%=basePath%>/portal/js/onlinehelp/comment.js"></script>
  ...
  <style type="text/css">
    ...
    <body>
		...
		<span style="border: #dae7f6 1px solid;"><span style="border: #dae7f6 1px solid;" title="审核" class="del_btn" onclick="auditComment()" onmouseover="doChangBg(this)" onmouseout="doReturn(this)"><a href="#">审核</a></span><span style="border: #dae7f6 1px solid;">｜</span></span>
    	</div>
   		<div id = 'newID'></div>
   </body>
</html>
...
doHiddenAll=function(){
	...
	if(left_tree.style.display=="none"&&top.offsetHeight == 0 ){
		...
		Ext.getCmp("auditcomment").setWidth(document.body.clientWidth-30);
		Ext.getCmp("doneGrid").setWidth(document.body.clientWidth-30);
		Ext.getCmp("todoGrid").setWidth(document.body.clientWidth-30);
	}
	if(left_tree.style.display=="block"&&top.offsetHeight != 0)	{
		...
		Ext.getCmp("auditcomment").setWidth(document.body.clientWidth-treeWidth);
	}
}
...
//-------------------问题类型------------------------------------------
	...
	var questionArea = new Ext.form.ComboBox({
		...
		editable: false,
		store: areastore,
		hidden:commentset==1?false:true,
		emptyText: '请选择问题地区',
		mode:'local',
		triggerAction: 'all',
		width:130,
