...
Ext.lt.formula.formulaconfig=function(service,config){
	...
	function createWin(d){
		...
		h.push('<div style="width:800px"><center><button class=\"btn\" onclick="Ext.lt.message.send(\'formulaconfig\',\'save\')">确认</button>&nbsp;&nbsp;<button class=\"btn\" onclick="wn.hidden();" >取消</button></center></div>');
		d.innerHTML=h.join('');
		wn.draw(d);
		buildFormulaTree();
		buildButton();
	...
	var celltree=Ext.lt.RCP.asyncall(service, "loadFormulasCell",  null);
	var cellqtree=null;
	function buildCellsTree(){
		var qtree=new Ext.lt.Qtree({
			...
			on:{'nodeclick':function(qtree,node){
					addFormula({code:"cells("+node.data.itemid+")",name:"cells("+node.data.name+")"});
			}}
		});
		qtree.draw(document.getElementById('formula_cells'));
		cellqtree=qtree;
	}
	var fttree=new Ext.lt.recordset();
	var ftqtree=null;
	var selectNode=null;
	var formulas=new Ext.lt.formulas();
	
	function buildFormulaTree(){
		var formulasInfs=Ext.lt.RCP.asyncall(service, "loadFunctions",null);
		fttree.addData(formulasInfs);
		var qtree=new Ext.lt.Qtree({
			data:fttree,
			outformartfn:function(node){
				return '<a title=\"'+node.description+"--"+node.product+'\">'+node.showName+'</a>';
			},
			field:{id:'name',showName:'showName',description:'description'},
			viewmodel:'list',
			on:{'nodeclick':function(qtree,node){
					addFormula({code:node.data.name+"(",name:node.data.showName+"("});
	...
	config.draw=function(d){
		var html=[];
		html.push("<div layout='{h:{fit:20},w:{fit:true}}'><button class=\"btn\" onclick='Ext.lt.message.send(\"formulaconfig\",\"test\")'>测试</button>&nbsp;&nbsp;<button class=\"btn\" onclick='Ext.lt.message.send(\"formulaconfig\",\"add\")'>新增</button>&nbsp;&nbsp;<button class=\"btn\" onclick='Ext.lt.message.send(\"formulaconfig\",\"del\")'>删除</button></div><div layout='{h:{fit:-20},w:{fit:true}}'></div><div class='winfitmode' class='winfitmode' style='height:500px;width:800px;overflow:hidden;'></div>");
	...
	Ext.lt.message.hook('formulaconfig','test',function(){

		Ext.lt.RCP.call(service,"test",null,function(resp){
			alert(resp);
		});
	...
	Ext.lt.message.hook('formSelect','onchange',function(){
		  ftqtree.setFilter([{field:'product',values:['PORTAL_FRAMEWORK_VERSION',_formula.product]}]);
		cellqtree.setFilter([{field:'product',values:['PORTAL_FRAMEWORK_VERSION',_formula.product]}]);
	});
	return config;
}