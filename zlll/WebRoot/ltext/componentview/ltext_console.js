Ext.lt.console.component.console = new function () {
	...
	this.showPageMangeData = function (datas) {
		...
		for (var i = 0; i < datas.length; i++) {
			loaddata[i] = new Array(datas[i].url, datas[i].page, datas[i].template, datas[i].identifier);
		}
		this.pageMangeDataDS.loadData(loaddata);
	};
	this.pageMangeDataDS = new Ext.data.Store({reader:new Ext.data.ArrayReader({}, [{name:"url"}, {name:"page"}, {name:"template"}, {name:"identifier"}])});
	this.sm = new Ext.grid.CheckboxSelectionModel({singleSelect:true});
	this.pageManageDataModel = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(), this.sm, {header:"url", width:200, sortable:true,menuDisabled:true, locked:false, dataIndex:"url", renderer:function (value) {
		return value + ".page";
	}}, {header:"\u8fd4\u56de\u5730\u5740", width:200, sortable:true,menuDisabled:true, locked:false, dataIndex:"page"}, {header:"\u9009\u62e9\u6a21\u7248ID", width:200, sortable:true,menuDisabled:true, locked:false, dataIndex:"template"}, {header:"\u662f\u5426\u6709\u4e2a\u6027\u8bdd\u8bbe\u7f6e", width:200, sortable:true,menuDisabled:true, dataIndex:"identifier", renderer:function (value) {
	...
	this.PageMangeData = new Ext.grid.GridPanel({layout:"fit", iconCls:"grid", border:0, store:this.pageMangeDataDS, cm:this.pageManageDataModel, sm:this.sm, frame:true, tbar:[{text:"\u65b0\u5efa", iconCls:"select", pressed:true, handler:function () {
		Ext.lt.console.component.console.templateWin(true);
	...
	//	alert("\u6682\u4e0d\u652f\u6301\u4fee\u6539\u529f\u80fd");
		...
		var selectrow = Ext.lt.console.component.console.PageMangeData.getSelectionModel().getSelected();
		Ext.lt.RCP.server(Ext.lt.console.component.console.server, "remove",  selectrow.data.url , function (resp) {
			if (resp!=null) {
				alert("\u5220\u9664\u6210\u529f");
				var data =resp;
				Ext.lt.console.component.console.showPageMangeData(data);
	...
	this.pagemanager = function (service, config) {
		//获取页面信息
		Ext.lt.console.component.console.server = 'pagemanagerserver';
		Ext.lt.RCP.server(Ext.lt.console.component.console.server, "load", null, function (resp) {
			var data =resp;
			Ext.lt.console.component.console.showPageMangeData(data);
			Ext.getCmp("pagemanager").doLayout();
		});
		//获取栏目内容
		Ext.lt.RCP.server(Ext.lt.console.component.console.server, "loadcomnents", null, function (resp) {
			this.comnents = resp[0];
			this.comnenttype = resp[1];
		});
		//获取模版信息
		Ext.lt.RCP.server(Ext.lt.console.component.console.server, "loadtemplates", null, function (resp) {
			this.templates = resp;
		});
		Ext.lt.console.component.console.PageMangeData.region = "center";
		var ret = {id:"pagemanager", title:"\u9875\u9762\u914d\u7f6e", border:0,layout:"fit", items:[Ext.lt.console.component.console.PageMangeData]};
		ret.getname=function(){return ret.title;}
		return ret;
	};
	this.templateWin = function (boo) {
		...
		if (Ext.getCmp("templateWin") == null) {
			...
			//		selecttemplatepanel.add(box);
				innerhtml += "<div onmouseout=\"Ext.lt.console.component.console.templatesBGColor('1',this);\" onmousemove=\"Ext.lt.console.component.console.templatesBGColor('2',this);\" onclick=\"Ext.lt.console.component.console.templatesBGColor('3',this,function(){Ext.lt.console.component.console.seltemp='" + templates[i].ID + "'});\" style='width:130,height:130;float:left;background-color: #CEDAEA'>&nbsp;<img width=100 height=100 style='filter:alpha(opacity=30);' src='" + templates[i].DEMOPIC + "'/>&nbsp;</div>";
			}
			...
			new Ext.Window({closeAction:"hide", title:"\u914d\u7f6e\u9875\u9762", width:700, height:400, layout:"column", plain:true, id:"templateWin", items:[{columnWidth:1, border:false, layout:"column", items:[{columnWidth:0.5, layout:"form", border:false, items:[this.urlfieldLabel]}, {columnWidth:0.5, layout:"form", border:false, items:[this.namefieldLabel]}]}, {columnWidth:1, layout:"form", items:this.jspfieldLabel}, selecttemplatepanel], buttons:[{text:"\u53d6\u6d88", handler:function () {
				Ext.lt.console.component.console.seltemp == null;
				Ext.getCmp("templateWin").hide();
				Ext.lt.console.component.console.clearMainBodyPanel();
			}}, {text:"\u4e0b\u4e00\u6b65", handler:function () {
				if (Ext.lt.console.component.console.seltemp == null) {
					alert("\u8bf7\u9009\u62e9\u9875\u9762\u6a21\u7248");
					return;
				}
				Ext.lt.console.component.console.config.url = Ext.getCmp("pageMange_url").getValue();
				Ext.lt.console.component.console.config.name = Ext.getCmp("pageMange_name").getValue();
				Ext.lt.console.component.console.config.page = Ext.getCmp("pageMange_page").getValue();
				Ext.lt.console.component.console.config.template = Ext.lt.console.component.console.seltemp;
				Ext.getCmp("templateWin").hide();
				Ext.lt.console.component.console.defComnentWin();
			}}]});
		}
		...
		if (tab == null) {
			...
			for (var i = 0; i < comnents.length; i++) {
				if (comnents[i].COMPONENTCLASS == type) {
					html += "<div style='width:130,height:130;float:left;background-color: #CEDAEA' ondblclick=\"Ext.lt.console.component.console.removeDefComponentDemo('" + type + "','" + comnents[i].VIEWID + "','" + comnents[i].DEMOPIC + "','" + comnents[i].REGION + "');\"  onclick=\"Ext.lt.console.component.console.templatesBGColor('3',this,function(){Ext.lt.console.component.console.setDefComponentDemo('" + type + "','" + comnents[i].VIEWID + "','" + comnents[i].DEMOPIC + "','" + comnents[i].REGION + "');});\" onmouseout=\"Ext.lt.console.component.console.templatesBGColor('1',this);\" onmousemove=\"Ext.lt.console.component.console.templatesBGColor('2',this);\">&nbsp;<img width=100 height=100 style='filter:alpha(opacity=30);' src='" + comnents[i].DEMOPIC + "'/>&nbsp;</div>";
				}
			}
			html += "</div>";
			...
			if (tab == null) {
				...
				for (var i = 0; i < comnents.length; i++) {
					if (comnents[i].COMPONENTCLASS == comnenttype[j].COMPONENTCLASS) {
						html += "<div style='width:130,height:130;float:left;background-color: #CEDAEA' ondblclick=\"Ext.lt.console.component.console.createComponent('" + comnents[i].COMPONENTID + "');\"  onclick=\"Ext.lt.console.component.console.templatesBGColor('3',this);\" onmouseout=\"Ext.lt.console.component.console.templatesBGColor('1',this);\" onmousemove=\"Ext.lt.console.component.console.templatesBGColor('2',this);\">&nbsp;<img width=100 height=100 style='filter:alpha(opacity=30);' src='" + comnents[i].DEMOPIC + "'/>&nbsp;</div>";
					}
				}
				html += "</div>";
		...
		if (Ext.getCmp("defComnentWin") == null) {
			...
			new Ext.Window({closeAction:"hide", title:"\u914d\u7f6e\u9875\u9762", width:700, height:400, layout:"column", plain:true, id:"defComnentWin", items:[{columnWidth:1, border:false, layout:"column", items:[{columnWidth:0.5, layout:"form", border:false, items:[this.urlfieldLabel_lab]}, {columnWidth:0.5, layout:"form", border:false, items:[this.namefieldLabel_lab]}]}, {columnWidth:1, layout:"form", items:this.jspfieldLabel_lab}, this.selectDefComnent], buttons:[{text:"\u53d6\u6d88", handler:function () {
				...
				if (Ext.getCmp("defComnentWin").buttons[2].getText() == "\u4e0b\u4e00\u6b65") {
					Ext.getCmp("defComnentWin").hide();
					Ext.lt.console.component.console.setConfigManage();
					Ext.lt.console.component.console.templateWin(false);
				} else {
				...
				if (Ext.getCmp("defComnentWin").buttons[2].getText() == "\u4e0b\u4e00\u6b65") {
					...
					Ext.lt.console.component.console.setConfigManage();
					if (Ext.lt.console.component.console.savePageValidation()) {
						Ext.lt.RCP.server(Ext.lt.console.component.console.server, "savePage", Ext.lt.console.component.console.config, function (resp) {
							if (resp == true) {
								alert("\u4fdd\u5b58\u6210\u529f");
								Ext.getCmp("selectDefComnent_tab").setVisible(true);
								Ext.getCmp("selectCommonComnent_tab").setVisible(false);
	...
	this.createComponent = function (comid) {
		for (var i = 0; i < comnents.length; i++) {
			if (comnents[i].COMPONENTID == comid) {
				...
				new Ext.lt.util.createScript( comnents[i].JSLIB, "console", function () {
					eval("Ext.lt.console.component.console.componentDeploy=new " + objname + ".deploy();");
					Ext.lt.console.component.console.componentDeploy.show(com, function (cof, service) {
						if (Ext.lt.console.component.console.config.main == null) {
							Ext.lt.console.component.console.config.main = new Array();
						}
						var obj = new Object();
						...
						companel.on("render", function () {
							companel.getEl().on("dblclick", function () {
								var arr = Ext.lt.console.component.console.config.main;
								var boo = false;
						...
						Ext.lt.console.component.console.config.main.push(obj);
					});
				});
				break;
			}
	...
	this.savePageValidation = function () {
		var config = Ext.lt.console.component.console.config;
	...
	this.setConfigManage = function () {
		Ext.lt.console.component.console.config.url = Ext.getCmp("pageMange_url_lab").getValue();
		Ext.lt.console.component.console.config.name = Ext.getCmp("pageMange_name_lab").getValue();
		Ext.lt.console.component.console.config.page = Ext.getCmp("pageMange_page_lab").getValue();
	};
