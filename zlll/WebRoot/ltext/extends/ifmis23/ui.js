...
Ext.fasp.ui.datatable=Ext.lt.createComponent(function(cfg){
	...
	function init(){
		...
		config.className="dttheme_ifmis";
		config.selectlinestyle="#CBDAF2";
		config.headsort=true;
		config.allowClock=true;
		config.isEdit=false;
	...
	function _draw(config){
		...
		config.table = new Ext.lt.datatable35(config.rs);
		//设置列
		...
		for(var i=0;i<columns.length;i++){
			...
			if(columns[i].show!=null){
				...
				if(columns[i].source==1){
					columns[i].codeShow=checkCodeShowFlag(columns[i].sourceelement);
					columns[i].fn=function(l,c,rs,v){
						var o={id:this.name,version:"3.0",config:config,codeShow:this.codeShow};
						o.show=showElement;
			...
			// 有来源数据项则已文本方式显示
			if(columns[i].sourceelement!=null && columns[i].sourceelement!='AMT') columns[i].datatype='S';
...
Ext.fasp.ui.queryform=Ext.lt.createComponent(function(config){
	...
	function initConfigWindow(){
		...
		_config_table.setClassName('dttheme_ifmis');
