...
Ext.lt.formulaconfig=function(service,config){
	...
	function createWin(d){
		...
		var h=[];
		h.push('<ul>');
		h.push('<li label="公式编码：" width=\"350px\" type=\"input\" maxLength=50 name=\"name\" redwordmode=\"on\" />');
		h.push('<li label="公式名称：" width=\"350px\" type=\"input\" maxLength=50 name=\"table\" redwordmode=\"on\" />');
		h.push('<li label="产品名称：" width=\"350px\" type=\"select\" maxLength=50 name=\"p\" redwordmode=\"on\" dataloader=\"ltdatatable.pro\" />');
		h.push('</ul>');
		h.push('<p>公式：');
		...
		h.push('</div>');
		h.push('</p>');
		h.push('<center><button class=\"btn\" onclick="wn.hidden();">确认</button>&nbsp;&nbsp;<button class=\"btn\" onclick="wn.hidden();" >取消</button></center>');
		h.push('</div>')
		d.innerHTML=h.join('');
		wn.draw(d);
		buildFormulaTree();
