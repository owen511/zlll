...
Ext.lt.formulas.initcells=function(cfg){
	...
	var cells={
		...
		setCells:function(str,ls,cs,wherefnstr){
			...
			cels[str]={}
			cels[str].wherefnset=wherefnstr;
			cels[str].cs=createRc(cs);
			cels[str].ls=createRc(ls);
			cels[str].getPushFn=function(){
				if(this._pushfn!=null)return this._pushfn;
				var _pushfn=[];
				_pushfn.push('function _pushfunction (rs,l,d,al,ac){');
				_pushfn.push('var ret=[];');
				var isRunFn=wherefnstr!=null;
				...
				for(var c=0;c<_cslength;c++){
					...
					if(!isNaN(cname)){
						...
						_c=names.indexOf(cname);
					}
					_pushfn.push('var v=d.',cname,',c=',_c,',cname=\'',cname,'\',bv=0;');
					if(isRunFn){
						_pushfn.push('if(',wherefnstr,'){');
					}
					_pushfn.push('if(!isNaN(v)){bv=parseFloat(v);}');
					_pushfn.push('ret.push(bv);');
					if(isRunFn){
						_pushfn.push('}');
					}
				}
				_pushfn.push('return ret');
				_pushfn.push('}');
				eval(_pushfn.join(''));
				this._pushfn=_pushfunction;
				return this._pushfn;
			}
			cels[str].fn=function(cells,al,ac){
				...
				for(var l=0;l<_lslength;l++){
					...
					var frs=this.getPushFn()(rs,l,d,al,ac);
...
Ext.lt.formulas.formulaInfo={'sum':{
		...
		name:"判断",
		promptinfo:"判断满足条件，如果满足返回一个，否则返回另一个(ifelse(test,true_value,false_value))",
