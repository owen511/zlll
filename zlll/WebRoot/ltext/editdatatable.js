...
Ext.lt.editdatatable=Ext.lt.createComponent(function(rs){
	...
	var _endEdite=function(save){
      ...
      if(isNaN(d)){
		...
		if(col.mapping!=null&&_mappingfilter){
	...
	function getEditorPosition(){
		...
		if(doc.clientHeight-e.clientY<200){
			p.top=st+e.clientY-e.offsetY-200-4
		}else{
	...
	function _init_edit_select(el,col,d,l,c){
		...
		function setSelectValue(){
			var col=_sel.col
			_editor.innerHTML=col._formartValue(_sel.firstChild.value);
		...
		function buildselect(b){
			...
			var _bindex=false;
				...
				if(str==null||str==''||_bindex||b){
					...
					*/
					_h.push("<option value='",cdata[i][ci],"'>",col._formartValue(cdata[i][ci]),"</option>");
				}
			}
			_h.push("</select>")
	...
	function setQreeSelectValue(code){
		var col=_editor.col
		_editor.innerHTML=col._formartValue(code);
		_editor.value=code
	}
	...
	function _initQtree(el,col,d,l,c,v){
		...
		_qtree=new Ext.lt.Qtree({
			field:{id:col.mapper.columns[0],sid:'supcode',name:'name',pinyin:'pinyin',level:"level"},
		...
		_qtree.colkey=col.mapper.columns[0];
