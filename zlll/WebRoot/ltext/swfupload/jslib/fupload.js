...
SWFUpload.prototype.fileDialogStart = function () {
	Ext.lt.message.send("fileupload","file_dialog_start_handler",this);
...
SWFUpload.prototype.fileQueued = function (file) {
	Ext.lt.message.send("fileupload","file_queued",{file:file,progress:this});
...
SWFUpload.prototype.fileDialogComplete = function (numFilesSelected, numFilesQueued, numFilesInQueue) {
	Ext.lt.message.send("fileupload","file_dialog_complete",this);
...
function fileQueued(file) {
	try {
		...
		//progress.setStatus("未上传...");
		progress.toggleCancel(true, this);
		progress.filename_div=this.temp_config.filename_div;
		Ext.lt.message.send("fileupload","fileQueued",progress);
	} catch (ex) {
		this.debug(ex);
	}
...
function uploadSuccess(file, serverData) {
	try {
		...
		progress.toggleCancel(false);
		Ext.lt.message.send('fileupload','upload_complete',this);
	} catch (ex) {
		this.debug(ex);
	}
}
...
function queueComplete(numFilesUploaded) {
	var files=[];
	for(var i=0;i<this.settings.file_upload_limit;i++){
	...
	Ext.lt.message.send("fileupload","queue_complete_submit",{files:files,progress:this});
}
