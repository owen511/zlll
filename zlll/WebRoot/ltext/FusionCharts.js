 ...
 *
 */
var infosoftglobal_abode_flash_visoin=true;
...
infosoftglobal.FusionCharts.prototype = {
	...
	render: function(elementId){
		...
		if((this.detectFlashVersion==1) && (this.installedVer.major < 10)){
			if (this.autoInstallRedirect==1&&infosoftglobal_abode_flash_visoin){
				//If we can auto redirect to install the player?
				var installationConfirm = window.confirm("由于您计算机上没有安装Adobe Flash Player 10.0版本以上，请您安装升级。 安装升级后请刷新该页面。");
				if (installationConfirm){
					infosoftglobal_abode_flash_visoin=false;
					window.location = _ROOT_PATH_+"/ltext/flashplayer.exe";
				}else{
					return false;
				}
			}else{
