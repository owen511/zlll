...
<html xmlns="http://www.w3.org/1999/xhtml">
				...
				JQ.ajax({
					...
					success: function(resp) {
						if (resp && typeof resp != "undefined") {
							var flag = confirm("您已经于" + resp[0].itime + "导入过此文件，是否继续导入？");
							if (flag) {
								formObject.submit();
							} else {
								window.close();
	...
	<br/>
		<form id="form1" name="form1" method="post"
			action="<%=request.getContextPath()%>/common/openFile1.do?mainmenu=<c:out value='${param.mainmenu}'/>&submenu=<c:out value='${param.submenu}'/>&vchtypeid=<c:out value='${param.vchtypeid}'/>&vchflagStr=<c:out value='${param.vchflagStr}'/>"
