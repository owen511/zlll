...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		<script>
			...
			var errorfilename = '<%=request.getAttribute("errorfilename")%>';
			queryform.action = "<%=request.getContextPath()%>/common/queryimportdata1.do?mainmenu=<c:out value="${param.mainmenu}"/>&submenu=<c:out value="${param.submenu}"/>&vchtypeid=<c:out value="${param.vchtypeid}"/>&vchflagStr=<c:out value="${param.vchflagStr}"/>&errorfilename="+errorfilename;	
			queryform.submit();
		</script>
	</head>
