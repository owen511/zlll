...
function MzTreeView(Tname)
{
  ...
  //【property】
  this.rootshow=false;
  this.naturalobj=new Object();
  this.editobj=new Object();
  this.url      = "#";
  this.target   = "_self";
  this.name     = Tname;
  this.wordLine = false;
  ...
  //专门针对checkbox
  this.getElementById1 = function(id)
  {
    if (typeof(id) != "string" || id == "") return null;
    if (document.getElementById){
    	var getobj=document.getElementById(id)
    	//eval("this.editobj."+getobj.value+"=''");
    	this.editobj[getobj.value]="";
    	//this.editobj.(getobj.value)
    	return getobj;
    }
    if (document.all) return document.all(id);
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  var HTML = "<DIV class='mztreeview' noWrap='True'><NOBR>"+HTMTH;
  if(!isRoot)
  {
  ...
  if( node.ctrl.length > 0 ) HTMCHK = "<input type='checkbox' name='"+ node.ctrl +"' value='"+ sid +
  	"' id='"+ this.name +"_checbox_"+ id +"' style='height:15px;"+((isNaN(sid)&&!this.rootshow)?"display:none;":"")+"'"+
  	" onclick=\""+ this.name +".boxonclick('"+ node.id +"')\" "+ HTMCHKED +">";	
  ...
  HTML += HTMCHK + "<A "+
    ...
    "target='"+ target +"' "+
    "title='"+ node.hint +"' "+
    ((this.name=="tree"&&!isNaN(sid))?"oncontextmenu = 'showmenuie5()' ":"")
    +
...
MzTreeView.prototype.selParent = function(node)
{
	if(node.parentId != "0")
	{
		var pnode = node.parentNode;
		var ckboxID = this.name +"_checbox_"+ pnode.id;
		var ckbox = this.getElementById1(ckboxID);
		if(ckbox.checked)
			return;
		else{
			ckbox.checked=true;
			this.selParent(pnode);
		}
	}else{
		return;
	}
...
MzTreeView.prototype.boxonclick=function (id){
	this.focusLink(id);
	if(event.srcElement.checked)
		this.selParent(this.currentNode);
	else{
		this.cancelcheck(this.currentNode);
	}	
}
//选中各级子节点
MzTreeView.prototype.checkall=function(node){
	if(node==undefined){
		node=this.currentNode;
		this.getElementById1(this.name+"_checbox_"+node.id).checked=true;
	}
	this.selParent(node)
	if(node.hasChild){
		if(!node.isExpand)
		...
		for(var i=0;i<node.childNodes.length;i++){
				this.getElementById1(this.name+"_checbox_"+node.childNodes[i].id).checked=true;
				this.checkall(node.childNodes[i]);
		}
	}
}
//选中下级子节点
MzTreeView.prototype.checknext=function(){
	this.getElementById1(this.name+"_checbox_"+this.currentNode.id).checked=true;
	this.selParent(this.currentNode)
	if(this.currentNode.hasChild){
		if(!this.currentNode.isExpand)
			this.expand(this.currentNode.id)
		for(var i=0;i<this.currentNode.childNodes.length;i++){
			this.getElementById1(this.name+"_checbox_"+this.currentNode.childNodes[i].id).checked=true;
			this.cancelcheck(this.currentNode.childNodes[i]);
		}
	}
}
//取消选中子节点
MzTreeView.prototype.cancelcheck=function (node){
	if(node==undefined){
		node=this.currentNode;
	}
	this.getElementById1(this.name+"_checbox_"+node.id);
	if(node.hasChild){
		if(!node.isExpand)
			this.expand(node.id)
		for(var i=0;i<node.childNodes.length;i++){
			this.getElementById1(this.name+"_checbox_"+node.childNodes[i].id).checked=false;
			this.cancelcheck(node.childNodes[i]);
		}
	}
	if(node.isExpand)
		this.expand(node.id)
}
MzTreeView.prototype.getSelectedNodes=function (){
	var chedobj=new Object();
	var chedarray=new Array();
	for(var pram in this.naturalobj){
		if(this.naturalobj[pram]!=this.editobj[pram]){
			chedobj[pram]="";
		}
	}
	var es=document.getElementsByName("sel");
	for(var i=0;i<es.length;i++){
		if (es[i].checked&&!isNaN(es[i].value)){
			chedobj[es[i].value]="";
		} 
	}
	for(var pra in chedobj){
		chedarray.push(pra);
	}
	return chedarray;
}
MzTreeView.prototype.getRemoveNodes=function (){
	var chedobj=new Object();
	...
	for(var pram in this.naturalobj){
		if(this.naturalobj[pram]!=chedobj[pram]){
			removearray.push(pram);
		}
	}
	return removearray;
}

MzTreeView.prototype.getAddNodes=function (){
	var chedobj=new Object();
	...
	for(var pram in chedobj){
		if(chedobj[pram]!=this.naturalobj[pram]){
			addarray.push(pram);
		}
	}
	return addarray;
}
//var addHtml="<STYLE> .skin0 {        BORDER-RIGHT: black 1px solid; BORDER-TOP: black 1px solid; VISIBILITY: hidden; BORDER-LEFT: black 1px solid; WIDTH: 100px; CURSOR: default; LINE-HEIGHT: 20px; PADDING-TOP: 4px; BORDER-BOTTOM: black 1px solid; FONT-FAMILY: \"宋体\"; POSITION: absolute; BACKGROUND-COLOR: menu; TEXT-ALIGN: left    }    .skin1 {        BORDER-RIGHT: buttonhighlight 2px outset; BORDER-TOP: buttonhighlight 2px outset; FONT-SIZE: 10pt; VISIBILITY: hidden; BORDER-LEFT: buttonhighlight 2px outset; WIDTH: 120px; CURSOR: default; PADDING-TOP: 4px; BORDER-BOTTOM: buttonhighlight 2px outset; FONT-FAMILY: \"宋体\"; POSITION: absolute; BACKGROUND-COLOR: menu; TEXT-ALIGN: left    }    .menuitems {        PADDING-RIGHT: 1px; PADDING-LEFT: 10px; PADDING-BOTTOM: 2px; PADDING-TOP: 2px    }    </STYLE>    <scr"+"ipt language=\"javascr"+"ipt\" id=\"clientEventHandlersJS\"> function document_oncontextmenu() { return false; } </scr"+"ipt> <scr"+"ipt language=\"javascr"+"ipt\" event=\"oncontextmenu\" for=\"document\"> return document_oncontextmenu() </scr"+"ipt> <scr"+"ipt language=javascr"+"ipt>    var menuskin = \"skin1\";     function getEvent(){           if(document.all)    return window.event;                   func=getEvent.caller;                       while(func!=null){                   var arg0=func.arguments[0];               if(arg0){                   if((arg0.constructor==Event || arg0.constructor ==MouseEvent)                       || (typeof(arg0)==\"object\" && arg0.preventDefault && arg0.stopPropagation)){                           return arg0;                   }               }               func=func.caller;           }          return null;    }       function showmenuie5() {    var event=arguments[0] || window.event;    var rightedge = document.body.clientWidth-event.clientX;    var bottomedge = document.body.clientHeight-event.clientY;    var event=arguments[0] || window.event;    var rightedge = document.body.clientWidth-event.clientX;    var bottomedge = document.body.clientHeight-event.clientY;    if (rightedge <ie5menu.offsetWidth)        ie5menu.style.left = event.clientX-left_tree.offsetWidth - ie5menu.offsetWidth;    else       ie5menu.style.left = event.clientX-left_tree.offsetWidth;    if (bottomedge <ie5menu.offsetHeight)        ie5menu.style.top = event.clientY-95 - ie5menu.offsetHeight;    else       ie5menu.style.top = event.clientY-95;    ie5menu.style.visibility = \"visible\";    return false;    }    function hidemenuie5() {    ie5menu.style.visibility = \"hidden\";    }    function highlightie5(evt) {    var event=evt || window.event;    var element=event.srcElement || event.target;    if (element.className == \"menuitems\") {    element.style.backgroundColor = \"highlight\";    element.style.color = \"white\";       }    }    function lowlightie5(evt) {    var event=evt || window.event;    var element=event.srcElement || event.target;    if (element.className == \"menuitems\") {    element.style.backgroundColor = \"\";    element.style.color = \"black\";       }    }    function jumptoie5(evt) {    var event=evt || window.event;    var element=event.srcElement || event.target;    if (element.className == \"menuitems\") {    if (element.getAttribute(\"target\") != null)    {    window.open(element.getAttribute(\"url\"), element.getAttribute(\"target\"));    }    else   window.location = element.getAttribute(\"url\");       }    }    </scr"+"ipt>    <DIV class=skin0 id=ie5menu onmouseover=highlightie5(event)  onmouseout=lowlightie5(event)>    <DIV id =menuitems3 class=menuitems onclick=\"tree.checknext()\">选中下一级子节点</DIV> <DIV id =menuitems1 class=menuitems onclick=\"tree.checkall()\">选中各级子节点</DIV> <HR> <DIV id =menuitems2 class=menuitems onclick=\"tree.cancelcheck()\">取消选中子节点</DIV>   </DIV>    <scr"+"ipt type=\"text/javascr"+"ipt\">    ie5menu.className = menuskin;    document.body.onclick = hidemenuie5;    </scr"+"ipt>  "
//document.write(addHtml)

MzTreeView.prototype.uniq =function (){
	var temparray=this.scbarray;
	var i = 0, j = 0;
    while (undefined !== temparray[i]){
        j = i + 1;
        while(undefined !== temparray[j]){
