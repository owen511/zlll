...
Zapatec.Tree.prototype.init = function (config) {
	...
	this.rootNode = new Zapatec.Tree.Node({tree:this, parentNode:null, level:0, isRootNode:true, eventListeners:this.config.eventListeners});
	...
	if (this.config.editable) {
		if (Zapatec.EditInline) {
			var self = this;
			this.editInline = new Zapatec.EditInline({editAsText:this.config.editAsText, eventListeners:{showStart:function () {
...
Zapatec.Tree.prototype.configure = function (objArgs) {
	...
	if (this.config.saveState && (!this.config.saveId || typeof (this.config.saveId) != "string" || this.config.saveId.length == 0)) {
...
Zapatec.Tree.prototype.reconfigure = function (objArgs) {
	if (objArgs.theme) {
		Zapatec.Utils.removeClass(this.container, this.getClassName({prefix:"zpTree", suffix:"Container"}));
	}
	Zapatec.Tree.SUPERclass.reconfigure.call(this, objArgs);
	Zapatec.Utils.addClass(this.container, this.getClassName({prefix:"zpTree", suffix:"Container"}));
};
...
Zapatec.Tree.prototype.displayErrorSource = function (oError) {
	alert("The tree's data source, " + this.config.source + " does not contain valid data.\n" + oError.errorDescription);
};
...
Zapatec.Tree.prototype.makeNode = function (html, type) {
	...
	if (html) {
		Zapatec.Transport.setInnerHtml({html:html, container:node});
	}
	return node;
};
...
Zapatec.Tree.prototype.attachNavigation = function () {
	...
	Zapatec.Utils.addEvent(document, (Zapatec.is_ie ? "keydown" : "keypress"), function (evt) {
...
Zapatec.Tree.prototype.keyEvent = function (evt) {
	...
	if (this.config.keyboardNavigation && (!this.config.editable || this.config.editable && !this.editInline && !this.editInline.selectedNode)) {
		switch (res.charCode) {
		  case 32:
			if (this.config.putCheckboxes) {
...
//wy 展开设置节点curpose值
Zapatec.Tree.Node.prototype.setCntForExpand =function(){
	...
	for(var i=0;i<this.children.length;i++){
		curnum +=this.children[i].curpose;
	}
	this.curpose +=	curnum;
    var parentNode = this.config.parentNode;
	if(this.config && parentNode){
		parentNode.curpose += this.curpose;
	}
}
//wy 收缩设置节点curpose值
Zapatec.Tree.Node.prototype.setCntForClose =function(){
	var isExpanded =  this.data.isExpanded;
	if(isExpanded){
		return;
	}
	var parentNode = this.config.parentNode;
		if(this.config && parentNode){
			parentNode.curpose -= (this.curpose-1);
		}
	this.curpose =1;
}
Zapatec.Tree.Node.prototype.curpose;
...
Zapatec.Tree.Node.prototype.create = function () {
	...
	content.push("'>");
	content.push("<table class='tree-table' cellpadding='0' cellspacing='0' style='border:0; line-height:20px;'><tbody><tr>");
	if (this.hasSubtree()) {
		content.push("<td nowrap style='border:0;' id='");
		content.push("zpTree");
	...
	if (this.config.tree.config.defaultIcons || (this.hasSubtree() && (this.data.collapsedIcon || this.data.expandedIcon || this.data.fetchingIcon) || !this.hasSubtree() && this.data.elementIcon)) {
		content.push("<td nowrap style='border:0;'");
		...
		content.push("'>");
		if (this.data.collapsedIcon || this.data.expandedIcon || this.data.fetchingIcon || this.data.elementIcon) {
			if (this.hasSubtree()) {
...
Zapatec.Tree.Node.prototype.afterCreate = function () {
	...
	if (this.hasSubtree()) {
		this.signElement = this.iconElement ? this.iconElement.previousSibling : this.labelContainer.childNodes[0].childNodes[0].childNodes[0].childNodes[0];
	...
	if (this.data.collapsedIcon || this.data.expandedIcon || this.data.fetchingIcon || this.data.elementIcon) {
		if (this.iconElement && this.iconElement.childNodes.length != 0) {
			var tmp = [this.iconElement.childNodes[0], this.iconElement.childNodes[1], this.iconElement.childNodes[2]];
			if (this.hasSubtree()) {
				for (var ii = 0; ii < 2; ii++) {
					...
					if (tmpIcon.className.indexOf("collapsedIcon") >= 0) {
						...
						if (tmpIcon.className.indexOf("expandedIcon") >= 0) {
							this.expandedIcon = tmpIcon;
						} else {
							if (tmpIcon.className.indexOf("fetchingIcon") >= 0) {
...
Zapatec.Tree.Node.prototype.createChildren = function () {
	if (!this.hasSubtree() || this.hasSubtree() && this.isChildrenCreated == true) {
		return null;
	}
...
Zapatec.Tree.Node.prototype.initChildren = function () {
	...
	for (var ii = 0; ii < this.data.children.length; ii++) {
		this.children.push(new Zapatec.Tree.Node({tree:this.config.tree, parentNode:this, level:this.config.level + 1, source:this.data.children[ii], sourceType:"json", eventListeners:this.config.eventListeners}));
	}
};
Zapatec.Tree.Node.prototype.isFirstNodeInBranch = function () {
	return this.config.isRootNode || this.labelContainer.parentNode.firstChild == this.labelContainer;
};
Zapatec.Tree.Node.prototype.isLastNodeInBranch = function () {
	return this.config.isRootNode || this.labelContainer.parentNode.lastChild == this.labelContainer || this.labelContainer.parentNode.lastChild == this.childrenContainer;
};
...
Zapatec.Tree.Node.prototype.getChildrenStats = function () {
	var res = {hasChecked:false, hasUnchecked:false};
...
Zapatec.Tree.Node.prototype.loadDataJson = function (objResponse) {
	...
	if (this.isCreated && (!this.config.tree.config.quick || this.config.tree.config.quick && this.isChildrenCreated)) {
...
Zapatec.Tree.Node.prototype.addNode = function (newChild, insertPosition) {
	if (newChild != null && newChild.nodeType && newChild.nodeType == 1 && newChild.nodeName.toLowerCase() == "li") {
	...
	if (this.isCreated || !this.config.tree.quick) {
		resultNode = new Zapatec.Tree.Node({tree:this.config.tree, parentNode:this, level:this.config.level + 1, source:newChild, sourceType:"json", eventListeners:this.config.eventListeners});
		if (this.isChildrenCreated) {
			...
			if (insertBeforeElement) {
				for (var ii = 0; ii < nodes.length; ii++) {
					this.childrenContainer.insertBefore(nodes[ii], insertBeforeElement);
				}
			} else {
				for (var ii = 0; ii < nodes.length; ii++) {
...
Zapatec.Tree.Node.prototype.getState = function () {
	var result = {label:this.data.label, isSelected:this.data.isSelected, attributes:Zapatec.Utils.clone(this.data.attributes), isChecked:this.data.isChecked};
...
Zapatec.Tree.Utils.convertLi2Json = function (liNode, compat) {
	if (liNode == null || liNode.nodeType != 1 || liNode.nodeName.toLowerCase() != "li") {
		return null;
	}
			...
			if (node.nodeName.toLowerCase() == "img") {
				if (compat) {
					...
					if (/\belementIcon\b/.test(node.className)) {
						...
						if (/\bexpandedIcon\b/.test(node.className)) {
							expandedIcon = node;
							continue;
						} else {
							if (/\bcollapsedIcon\b/.test(node.className)) {
								collapsedIcon = node;
								continue;
							} else {
								if (/\bfetchingIcon\b/.test(node.className)) {
									fetchingIcon = node;
									continue;
								}
							}
	...
	for (var ii = 0; ii < liNode.attributes.length; ii++) {
		...
		if (Zapatec.is_ie && liNode.getAttributeNode(attr.nodeName) && !liNode.getAttributeNode(attr.nodeName).specified) {
			continue;
		}
...
Zapatec.Tree.Utils.convertXml2Json = function (itemNode) {
	if (itemNode == null || itemNode.nodeType != 1 || itemNode.nodeName.toLowerCase() != "item") {
		return null;
	}
	...
	for (var ii = 0; ii < itemNode.childNodes.length; ii++) {
		...
		if (node.nodeName.toLowerCase() == "attribute" && node.getAttribute("name")) {
...
Zapatec.Tree.Utils.getPrevNode = function (prevSelected) {
	...
	if (index > 0) {
		...
		if (!prevSelected.config.parentNode.config.isRootNode) {
...
Zapatec.Tree.Utils.getNextNode = function (prevSelected) {
	...
	var nextNode = null;
	if (prevSelected.hasSubtree() && prevSelected.data.isExpanded && prevSelected.children.length > 0) {
		nextNode = prevSelected.children[0];
	} else {
		if (index < prevSelected.config.parentNode.children.length - 1) {
...
function getTreeSelected(treeObj) {
	var selects = treeObj.findAll(function (node) {
	...
	for (var i = 0; i < selects.length; i++) {
		result["value"] += selects[i].data.label + "; ";
		result["id"] += selects[i].data.id + ",";
		result["valuecode"] += selects[i].data.code + ",";
		result["isleaf"] += selects[i].data.isleaf + ",";
	}
	return result;
}
