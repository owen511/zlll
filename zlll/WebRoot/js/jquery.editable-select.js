...
(function(JQ) {
  var instances = [];
  JQ.fn.editableSelect = function(options) {
    ...
    if(settings.bg_iframe && !JQ.browser.msie) {
      settings.bg_iframe = false;
    };
    var instance = false;
    JQ(this).each(function() {
      var i = instances.length;
      if(JQ(this).data('editable-selecter') !== null) {
        instances[i] = new EditableSelect(this, settings);
        JQ(this).data('editable-selecter', i);
      };
    });
    return JQ(this);
  };
  JQ.fn.editableSelectInstances = function() {
    var ret = [];
    JQ(this).each(function() {
      if(JQ(this).data('editable-selecter') !== null) {
        ret[ret.length] = instances[JQ(this).data('editable-selecter')];
      };
    });
    return ret;
  };
  ...
  EditableSelect.prototype = {
    ...
    init: function(select, settings) {
      ...
      this.wrapper = JQ(document.createElement('div'));
      this.wrapper.addClass('editable-select-options');
      this.select = JQ(select);
	  this.width = this.select.width()+2;
      this.text = JQ('<input type="text">');
      this.text.attr('name', this.select.attr('name'));
	  //this.select.attr('name',this.select.attr('name')+"_select")
	  ...
	  if(this.settings.onblur) {
		 var context = this;
		 var blur = this.settings.onblur;
         this.text.bind("blur",function(event){
			 blur.call(this);
	  ...
	  if(this.settings.onkeyup) {
         this.text.bind("keyup", this.settings.onkeyup);
      };
      ...
      this.setWidths();
  	  this.select.remove(); 
      JQ(this.settings.datatable.parent.parentElement).append(this.wrapper);
    ...
    duplicateOptions: function() {
      var context = this;
      var option_list = JQ(document.createElement('ul'));
      this.wrapper.append(option_list);
      var options = this.select.find('option');
      options.each(function() {
        if(JQ(this).attr('selected')) {
          context.text.val(JQ(this).val());
          context.current_value = JQ(this).val();
        };
        var li = JQ('<li>'+ JQ(this).val() +'</li>');
          ...
          switch(e.keyCode) {
            ...
            case 13:
			  ...
			  if(value_found){
				  ...
				  context.text.val(current_value);
				  context.current_value = current_value;
				  if(typeof context.settings.onSelect == 'function') {
					context.settings.onSelect.call(context, current_value);
				  };
				  context.hideList();
				  //context.blur();
			  }
        ...
        function() {
          if(!value_found) {
            ...
            if(text == current_value) {
              value_found = true;
              context.clearSelectedListItem();
              context.selectListItem(JQ(this));
              context.scrollToListItem(JQ(this));
              return false;
    ...
    positionElements: function() {
	  ...
	  var offset1 = JQ(this.settings.datatable.parent.parentElement).offset();
      offset = { top: offset.top-offset1.top, left: offset.left-offset1.left };
    ...
    createBackgroundIframe: function() {
      var bg_iframe = JQ('<iframe frameborder="0" class="editable-select-iframe" src="about:blank;"></iframe>');
      JQ(document.body).append(bg_iframe);
      bg_iframe.width(this.width);
