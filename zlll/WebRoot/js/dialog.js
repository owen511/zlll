...
Dialog.prototype.init = function(){
  this.dlg = pwin.JQ("<dl id=\""+this.dlgName+"\" class=\"dialog\" style=\"display:none;\"><dt class=\"dialog_bar\"><span class=\"dialog_closeIcon\"></span><span class=\"dialog_title\"></span></dt><dd class=\"dialog_pane\"></dd></dl>");
  this.dlgMask = pwin.JQ("<div id=\""+this.dlgName+"_mask\" class=\"dialog_mask\" style=\"display:none\"></div>");
  ...
  this.dlgAlphaBorder = pwin.JQ("<div id=\""+this.dlgName+"_alphaborder\" class=\"dialog_alphaborder\" style=\"display:none\"></div>");
  pwin.JQ("body").append(this.dlg);
  pwin.JQ("body").append(this.dlgMask);
  pwin.JQ("body").append(this.dlgAlphaBorder);
...
Dialog.prototype._initStyle = function(title,style){
  ...
  if(style.model == ""){
    ...
    //if(document.documentElement){
      //this.dlgMask.css("width",pwin.JQ("body").width()+"px");
      //this.dlgMask.css("height",pwin.JQ("body").height()+"px");//ie bug?
  ...
  if(style.center){
    if (typeof document.body.style.maxHeight === "undefined") {//if IE 6
      ...
      this.dlg.css("top",(pwin.JQ("body").scrollTop()+top)+"px");
      this.dlg.css("left",(pwin.JQ("body").scrollLeft()+left)+"px");
...
Dialog.prototype.showDiv = function(title,style,divId,context){
  ...
  this.cType = "div";
  this.divContainer = pwin.JQ("#"+divId).parent();
  this.dlg.find(".dialog_pane").append(pwin.JQ("#"+divId));
  pwin.JQ("#"+divId).css("display","");
}
...
Dialog.prototype.showIframe = function(title,style,iframeUrl,context){
  ...
  var iframe = pwin.JQ("<iframe class=\"dialog_iframeContent\" width=\"100%\" height=\"100%\" src=\""+iframeUrl+"\" frameborder=\"0\"></iframe>");
...
Dialog.prototype.showSimple = function(title,style,content,buttons,context){
  ...
  var dlgContent = pwin.JQ("<div>"+content+"</div>");
  dlgContentContainer.append(dlgContent);
  var dlgButtons = pwin.JQ("<div></div>");
  dlgContentContainer.append(dlgButtons);
  for(var i=0;i<buttons.length;i++){
    var btn = pwin.JQ("<button type=\"button\">"+buttons[i].text+"</button>");
