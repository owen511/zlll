...
GridTree.prototype.loadData = function(obj) {
	...
	tc = jQuery.extend({
			...
			exchangeColor:false,//单双行交替显示颜色
			closeImg : '../../images/gridTree/minus.gif',//父级节点的关闭图标
			openImg : '../../images/gridTree/plus.gif',//父级节点的打开图标
			lazyLoadImg : '../../images/gridTree/plus2.gif',//懒加载时显示的图标
			blankImg : '../../images/gridTree/blank.gif',//空白节点
			noparentImg : '../../images/gridTree/blank.gif',//树叶节点的图标
			pageBarId : 'pageBarTable',//默认的分页栏表格对象的id为'pageBarTable'
			pageSize : 20,//默认翻页的每页显示10行
			...
			checkColumnDesc : '选择',//默认的选择列名为'请选择'
			...
			hadValueCode:null,//选中值的id
			hadData:null,	//已选中数据集
			hadRowFlag:false, //已创建过的行标识
			remake:false//重新创建树
		  }, obj);
	elct = _$(tc.el);
	headColumns = [];
...
GridTree.prototype.makeTable = function() {
	...
	if (tc.data != null) {
		...
		//已有选择数据
		if(tc.hadValueCode()&&!inQueryFlag){
			var param = {vchtypecode:'8003',hadValueCode:tc.hadValueCode().replace('N','')};	
			parent.show();
			jQuery.ajax({
				type : "POST",
				url : "/common/jump/list_program.do",
				async : true,
				data:param,
		...
		if(typeof objPars=="object"){
			param = JQ.extend(param,objPars);
		}
		parent.show();
		jQuery.ajax({
			type : "POST",
...
GridTree.prototype._makeTable = function(tree,data)
 {
	...
	if(!(tc.checkOption=='2'&&tc.allCheck))return;
	var ts=tree.getElementsByTagName('input');
...
GridTree.prototype._showLazyTable = function(tableTree, data) {
	this._clearContent();
	var datas = eval(tc.allDataInfoWithPageInfo.data);
	if(tc.hadData && eval(tc.hadData).length ==1){
		datas = eval(tc.hadData).concat(datas);
		//"-"含中线说明是默认值需要转换成id
		if(!tc.remake && parent.selvalue.indexOf("-")>-1)parent.selvalue = eval(tc.hadData)[0][tc.idColumn]
	}
	if(tc.hadData && eval(tc.hadData).length > 1){
		var path = "0,";
		for(var i =0 ;i<eval(tc.hadData).length;i++){
			path += eval(tc.hadData)[i][tc.idColumn]+",";
		}
		parent.selvalue = path.substring(0,path.length-1);
		var remake = true;
		getGridTree(remake);
		return;
	}
	...
	for(var i=0,j=datas.length;i<j;i++){
		...
		var _id = oneObj[tc.idColumn];
		var _code = oneObj.code;
		if(tc.hadValueCode() && (tc.hadValueCode().replace("N","")==_id || tc.hadValueCode().replace("C","")==_code)){
			if(!tc.hadRowFlag)
				tc.hadRowFlag = true;
			else 
				continue;
...
GridTree.prototype._resetPageBtns = function(){
	...
	if (pagingInfo.pagesCount == 1||pagingInfo.pagesCount == 0) {
		if(jQuery(f1t).hasClass('firstPage')){
			jQuery(f1t).removeClass("firstPage").addClass("disFirstPage");

		}
		if(jQuery(pre).hasClass('prevPage')){
			jQuery(pre).removeClass("prevPage").addClass("disPrevPage");

		}
		if(jQuery(lst).hasClass('lastPage')){
			jQuery(lst).removeClass("lastPage").addClass("disLastPage");
		}
		if(jQuery(nex).hasClass('nextPage')){
			jQuery(nex).removeClass("nextPage").addClass("disNextPage");
		}
		jQuery(f1t).attr("disabled",true);
		jQuery(nex).attr("disabled",true);
		jQuery(lst).attr("disabled",true);
		jQuery(pre).attr("disabled",true);
		if(pagingInfo.pagesCount==0){ //没数据把跳转按钮disabled
			 jQuery(tpg).attr("disabled",true);		
		}
	} else {
		...
		if (pagingInfo.currentPage == 1) {		
			...
			if(jQuery(lst).hasClass('disLastPage')){
				jQuery(lst).removeClass("disLastPage").addClass("lastPage");
			}
			if(jQuery(nex).hasClass('disNextPage')){
				jQuery(nex).removeClass("disNextPage").addClass("nextPage");
			}
			jQuery(f1t).attr("disabled",true);
			jQuery(pre).attr("disabled",true);
			jQuery(lst).attr("disabled",false);
			jQuery(nex).attr("disabled",false);
		} 
		//如果当前页就是最后一页，设置后一页和最后一页按钮为不可用样式
		else if (pagingInfo.currentPage == pagingInfo.pagesCount) {
			if(jQuery(f1t).hasClass('disFirstPage')){
				jQuery(f1t).removeClass("disFirstPage").addClass("firstPage");
			}
			if(jQuery(pre).hasClass('disPrevPage')){
				jQuery(pre).removeClass("disPrevPage").addClass("prevPage");
			}
			if(jQuery(lst).hasClass('lastPage')){
				jQuery(lst).removeClass("lastPage").addClass("disLastPage");
			}
			...
			jQuery(f1t).attr("disabled",false);
			jQuery(pre).attr("disabled",false);
			jQuery(lst).attr("disabled",true);
			jQuery(nex).attr("disabled",true);
		}
...
GridTree.prototype._initPageInfo = function()
{
	if (_serverPagingMode == 'client') {	
		...
		pagingInfo.allCount = Number(tc.allDataInfoWithPageInfo.total);
		pagingInfo.pageSize = tc.pageSize;
		pagingInfo.pagesCount = Math.ceil(pagingInfo.allCount
				/ pagingInfo.pageSize * 1.0);
		//后台json传串直接传来当前页数
		pagingInfo.currentPage = Number(tc.allDataInfoWithPageInfo.page);
	} 
	else {
		//全部的行数
...
GridTree.prototype._analyseData = function(msg)
{
	...
	if(_serverPagingMode=='server'){
		...
		if(_serverPagingMode == 'client'){
			...
			eval("tc.allDataInfoWithPageInfo=" + msg);
			data = eval(tc.allDataInfoWithPageInfo.data);
...
GridTree.prototype._pagingServerMakeTable = function(tree,msg)
{
	eval(" tc.allDataInfoWithPageInfo=" + msg);
	//得到后台传来的json数组
	var data = eval(tc.allDataInfoWithPageInfo.data);
	this._initData();
	// 首先分析数据
	this._analyseData(data);
	// 重新设置当前页
...
GridTree.prototype._addTitleHead = function(tableTree) {
	...
	if (tc.checkOption == '1' || tc.checkOption == '2') {
		...
		jQuery(checkCell).attr('id','countCell').width('5%').addClass('tablehead').addClass('checkCell');
		...
		if(tc.checkOption=='2'&&tc.allCheck)
		{
			...
			JQ(checkCell).click(tc.handleCheck);
		}
		else{
	...
	for (var ii=0;ii<lenlen;ii++) {
		...
		newSpan.innerHTML = "&nbsp;";
		newSpan.setAttribute('class','resizeDivClass');
		newSpan.setAttribute('onMouseDown','MouseDownToResize(this,\''+tc.columnModel[i].width+'\',\''+tc.width+'\')');
		newSpan.setAttribute('onMouseMove','MouseMoveToResize(this)');
		newSpan.setAttribute('onMouseUp','MouseUpToResize(this)');
		if (tc.columnModel[i].width != null
				&& tc.columnModel[i].width != '*')
			newCell.style.width = tc.columnModel[i].width;
		i = i + 1;
		jQuery(newCell).addClass('tablehead').attr('id',oneColumn.headerIndex).html(newSpan.outerHTML+oneColumn.header).appendTo(newRow);
		headColumns.push([oneColumn.headerCode,oneColumn.headerIndex]);
	}
...
GridTree.prototype._makePageBar = function(tableObj, index) {
	...
	//selectPagingCell.innerHTML = '每页<input type=text onkeyup="GridTree.prototype._reMakeTable(this.value);" id="_changePageSizeSel" size=4></input>条';
	var msgCell = pageBarTR.insertCell(pageBarTR.cells.length);
	// 显示的分页(当前页,总页数)
	msgCell.style.textAlign = 'right';
	msgCell.setAttribute('id', 'msgCell');
	msgCell.innerHTML = ["&nbsp;一级共",pagingInfo.allCount,"条,每页",pagingInfo.pageSize,"条,第" , pagingInfo.currentPage , "&nbsp;/&nbsp;"
			, pagingInfo.pagesCount , "页","<input type='text' style='width:30px' id='_pageNum' "
			, "onkeypress='return event.keyCode>=48&&event.keyCode<=57||event.keyCode==46'"
			, "onpaste='return !clipboardData.getData(\"text\").match(/\D/)'>"
			, "&nbsp;<button class='pagebarbtn jumpPage' onclick='GridTree.prototype._toPage(\"any\")' title='跳转' id='_toPageBtn'></button>"].join('');
	//如果是ext的样式.
	...
	//jQuery(newCell).height('30px');
	//newCell.appendChild(pageBar);
	JQ("#pageTagDiv").append(pageBar);
}
 ...
 */
GridTree.prototype._addOneLazyRowByData = function(index, rowData, level,nth,chkCode) {
	var newRow = document.createElement('tr');
	...
	this._addCheckOptionCell(rowData,newRow,chkCode);
	...
	newRow.ondblclick = function (){
		if (tc.checkOption == '2') {
			...
			setMulSelectedObj(_id);
		}else {
			parent.selectObj.id = _id;
			var pnode = myTree.getRowObjById(_id);
			var ppath = pnode.getAttribute('_node_path');
			var path = ppath.replace(/\_node/g,"")+","+_id;
...
GridTree.prototype._addOneLazyRowContent = function(rowData,newRow,_id,_isP,level){
	...
	for (; i < headColumns.length; i++) {
		...
		newSmallCell.setAttribute('_td_pro', headColumns[i][1]);
		jQuery(newSmallCell).addClass('content');
		var _t = rowData[headColumns[i][1]];
		// 在第一列进行图标的设置
		if (i == 0) {
			if (_t != '') {				
				var ct = level - 1;
				...
				for(var ii=0;ii<ct;ii++){
					ans.push(['<IMG ','src="',tc.blankImg,'" style="vertical-align:middle;vertical-align:middle"/>'].join(''));
				}
				// 如果是父亲节点
				if (_isP == '1') {
					ans.push(["<IMG id='img_"
							, _id, "' style='CURSOR: hand;vertical-align:middle' ",
							"onclick='javascript:_parentId=this.id;GridTree.prototype.openChildrenTable(this.id,this,event);' src='",
							tc.openImg , "'/>" ].join(''));
				} else {
						ans.push(["<IMG id='img_" , _id, "' src='" , tc.noparentImg , "' style='vertical-align:middle'/>"].join(''));					
				}	
		...
		// 其他列就直接设置内容即可
			...
			if (_t != '') {
				...
				if(showTypeDesc!=null){
					...
					if(headColumns[i][0]==""){
						newSmallCell.innerHTML = rowData[headColumns[i][1]]==null ? '' : "<div onmouseout='return nd();' onclick='return programShowOverlibTips(this.innerHTML,this);' class='divoverflow'>"+rowData[headColumns[i][1]]+"</div>";
					}else{
						newSmallCell.innerHTML=rowData[headColumns[i][0]]==undefined?'': "<div onmouseout='return nd();' onclick='return programShowOverlibTips(this.innerHTML,this);' class='divoverflow'>"+rowData[headColumns[i][0]]+"-"+rowData[headColumns[i][1]]+"</div>";
						 }
				}
			} else
				newSmallCell.innerHTML = '&nbsp;';
	...
	for (; i < headColumns.length; i++) {
		...
		if (i == 0) {			
			if (_t != '') {				
				...
				var ans = [];
				for(var ii=0;ii<ct;ii++){
					ans.push(['<IMG ','src="',tc.blankImg,'" style="vertical-align:middle"/>'].join(''));
				}
				// 如果是父亲节点
				if (_isP == '1') {
					ans.push(["<IMG id='img_"
					...
					if (findInArray(firstLevelNodes, '_node'+_id) == -1) {
						ans.push(["<IMG id='img_" , _id
								, "' src='" , tc.noparentImg , "' style='vertical-align:middle'/>"].join(''));
						newRow.style.display = 'none';
					}
 ...
 */
GridTree.prototype._toPage = function(operCode,curpage) {
    var reg = /^[0-9]*$/;
	if(tc.hadRowFlag)tc.hadRowFlag=false;
	var pnum = _$('_pageNum'),mdiv = _$('_msgDiv'),mcel = _$('msgCell'),tb = _$(tc.tableId);
	if(!reg.test(pnum.value.trim())) {alert("请输入正确页码！");pnum.focus();pnum.value="";return};
	switch(operCode){
		case 'first' :
			pagingInfo.currentPage = 1;
			break;
		case 'pre':
			pagingInfo.currentPage = (Number(pagingInfo.currentPage) - 1) < 1
			? 1
			: (Number(pagingInfo.currentPage) - 1);
			break;
		case 'next':
			pagingInfo.currentPage = (Number(pagingInfo.currentPage) + 1) > Number(pagingInfo.pagesCount)
			? Number(pagingInfo.pagesCount)
			: (Number(pagingInfo.currentPage) + 1);
			break;
		case 'last':
			pagingInfo.currentPage = Number(pagingInfo.pagesCount);
			break;
		default:
			var n = curpage == "" || curpage == undefined ? pnum.value : curpage;
			if (n == '' || n > Number(pagingInfo.pagesCount) || n==0) {
	...
	var num = Number(pagingInfo.currentPage);
	if (parent.chkAll != null && parent.chkAll.length>0) {
		var _pageNum = jQuery.inArray(pagingInfo.currentPage+"", parent.chkAll);
		if (_pageNum > -1) {
			jQuery("#_checkAll").attr("checked",true);
		} else {
			jQuery("#_checkAll").attr("checked",false);
		}
	} else {
		jQuery("#_checkAll").attr("checked",false);
	}
		...
		jQuery.ajax({
			...
			success : function(msg) {
				try{
					...
					mcel.innerHTML = ["一级共",pagingInfo.allCount,"条,每页",pagingInfo.pageSize,"条,第" , pagingInfo.currentPage , "&nbsp;/&nbsp;"
						, pagingInfo.pagesCount , "页","<input type='text' style='width:30px' id='_pageNum' "
						, "onkeypress='return event.keyCode>=48&&event.keyCode<=57||event.keyCode==46'"
						, "onpaste='return !clipboardData.getData(\"text\").match(/\D/)'>"
						, "&nbsp;<button class='pagebarbtn jumpPage' onclick='GridTree.prototype._toPage(\"any\")' title='跳转' id='_toPageBtn'></button>"].join('');
 ...
 * @param {行对象} newRow
 */
GridTree.prototype._addCheckOptionCell = function(rowData,newRow,chkCode){
	var _id = rowData[tc.idColumn];
	...
	if (tc.checkOption == '1') {
		...
		var _code = rowData.code;
		var chkflag = '';
		if(tc.hadValueCode()&&tc.hadValueCode().indexOf('N')<0&&(tc.hadValueCode() == _id || tc.hadValueCode().replace("C","")==_code)){
			chkflag = true;
		}
		else{
			chkflag = chkCode == _id ? true:'';
		}
		createRadio(checkCell,setOptionDisabled,'width:20px;border:0px;','_chks',_id,tc.handleCheck,'','',chkflag);
		jQuery(checkCell).appendTo(newRow);
		...
		//选择模式为3,A孩子选择父亲不自动选择,B孩子都没有选择则父亲自动不选择,C父亲选择孩子全部选择.
			...
			createCheckbox(checkCell,setOptionDisabled,'width:20px;border:0px;','_chks',_id,function(){
						GridTree.prototype.openChildrenTable(_id,this,event);
						GridTree.prototype._chooseChildrenNode(this);
						//GridTree.prototype._chooseParentNode(this); //jzy处理，选中父级，子级级联选中，其他情况相互独立。原始写法有问题，改造其他模式，按照模式3修改。
						//GridTree.prototype._cancelFaher(this);
						 if(tc.handleCheck) tc.handleCheck(this);
					},'','',defalutChoose);
		}
...
GridTree.prototype._addOneRowListerners = function(newRow){
	...
	if (tc.handler) {
		...
		newRow.onclick = function() {
			...
			jQuery(newRow).addClass("selectlight");
			//var chkflag = jQuery(newRow).find("INPUT:[type=checkbox]").attr("checked");
			//if(chkflag)
			_lastSelectRowId = newRow.id;
		};
	}
...
GridTree.prototype._verifyAjaxAns = function(msg){
	eval(" tempData=" + msg);
	var data = eval(tempData.data);
	// 验证输入的标示列的值是否在‘data’数组中存在
	var columnName = tc['idColumn'];
	if (data.length==0 || typeof data[0][columnName] == 'undefined') {
		return false;
	}
...
{
	var ansArr = parentToChildMap[nodeid];
	return ansArr;
}
...
GridTree.prototype.getSelectedRow = function() {
	var selectedId = _lastSelectRowId.split(",");
	var arrSelectedRow = [];
	for(var i = 0;i<selectedId.length;i++){
		arrSelectedRow.push(_$(selectedId[i]));
	}
 ...
 * 点击一个图标,打开其子节点
 */
GridTree.prototype.openChildrenTable = function(imgid,node,e,chkCode)
{	 

	var _id = imgid.replace('img_','').replace('_node','');
	var img = _$('img_'+_id);
	var _node1 =   _$('_node'+_id);
	if(_node1.getAttribute('_node_isparent')!='1'){ //没有子节点没必要继续走
		return;
	}
					...
					if (msg != null && msg != "") {
						try {
							...
							for(var i=datas.length-1;i>=0;i--){
								...
								var newRow = GridTree.prototype._addOneLazyRowByData(startIndex,datas[i],level,i+1,chkCode);
								if (parentToChildMap['_node' + _id]) {
									var arr = parentToChildMap['_node' + _id];
									arr.push('_node' + datas[i].itemid);
									parentToChildMap['_node' + _id] = arr;
								} else {
									var arr = [];
									arr.push('_node' + datas[i].itemid);
									parentToChildMap['_node' + _id] = arr;
...
GridTree.prototype.closeChildrenTable = function(imgid,node,e)
{
	...
	jQuery('tr[_node_parent=_node'+_id+']').each(function(i){
		...
		if(this.getAttribute('_node_level')!='1'){ //展开级次为2的节点收回。
 ...
 *得到当前页
 */
GridTree.prototype.getCurrentPage = function()
{
...
function createRadio(el,dis,sty,name,v,click,t,cssname,chk){
	if(chk && !inQueryFlag){
		jQuery(el).append(jQuery("<input type='radio' name='"+name+"' checked>")
		.attr({'style':sty,'userSetDisabled':dis,'value':v,'disabled':dis})
...
//拖动
function MouseDownToResize(obj,width,offsetW){ 
	setTableLayoutToFixed(); 
	obj.downWidth = width;
	obj.mouseDownX=event.clientX; 
	obj.pareneTdW=obj.parentElement.offsetWidth; 
	obj.pareneTableW=testTable.offsetWidth; 
	obj.offsetW = offsetW;
	obj.setCapture(); 
} 
function MouseMoveToResize(obj){ 
	...
	if(newWidth>0 && newWidth>obj.downWidth.replace("px","")) 
	{ 
		if(obj.pareneTableW*1+event.clientX*1-obj.mouseDownX > Number(obj.offsetW.replace("px",""))){
			obj.parentElement.style.width = newWidth; 
			testTable.style.width=obj.pareneTableW*1+event.clientX*1-obj.mouseDownX; 
		}
	} 
} 
...
{ 
	if(testTable.style.tableLayout=='fixed') return; 
	var headerTr=testTable.rows[0]; 
	...
	testTable.style.tableLayout='fixed'; 
} 