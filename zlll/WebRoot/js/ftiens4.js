  ...
  if (nc > 0) {
    ...
    for (i = 0 ; i < this.nChildren; i++) {
      if (typeof this.children[i].initialize == 'undefined') {
		  //document node was specified using the addChildren function
...
function drawFolder(insertAtObj) {
  ...
  if (browserVersion > 0) {
    auxEv = "<a style='cursor:hand' onclick='javascript:clickOnNode(\""+this.getID()+"\")'>";
  } else {
    auxEv = "<a>";
  }
  ...
  if (this.level>0) {
    if (this.isLastNode) {
        ...
        leftSide = leftSide + "<td valign=top>" + auxEv + "<img name='nodeIcon"
		           + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName
				   + "' width=16 height=22 border=0></a></td>";
	} else {
      leftSide = leftSide + "<td valign=top background=" + ICONPATH + "ftv2vertline.gif>"
	             + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id
				 + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>";
	}
  }
  ...
  if (WRAPTEXT) {
      ...
      docW = docW + "</td>" + this.prependHTML + "<td valign=middle nowrap=\"nowrap\" width=100%>";
  }
  if (USETEXTLINKS) {
...
function linkFolderHTML(isTextLink)
{
  ...
  // 2008-8-31 wengyuedong 修改
  // 当不传链接时保留文字的电击事件
  else{
//    docW = docW + "<a>";
      docW = docW + "<a style='cursor: hand'";
...
function drawItem(insertAtObj)
{
  ...
  var fullLink = "href=\""+this.link+"\" target=\""+this.target+"\" onClick='clickOnLink(\""+this.getID()+"\",\""+tempLink+"\",\""+this.target+"\");return false;\'";
...
function folderMstr(domObj)
{
  if (browserVersion == 1 || browserVersion == 3) {
    /*
     * Cracked by RaTKiNG 2008-02-29
     * 注释掉下面5行
     * 用于检测树的上方必须含有一个链接：href="http://www.treemenu.net/"，
     * 即链接的第15和第16个字符必须是“em”。如果检测不到，就不显示树了。
     */
//    if (t==-1)
//      return;
...
function clickOnLink(clickedId, target, windowName) {
	if(target!=null && target.indexOf("isMenu=yes")==-1){
		target += ((target.indexOf("?")==-1)?"?":"&")+"isMenu=yes";
	}
...
function preLoadIcons() {
	...
	for(var i=0;i<arImageSrc.length;i++){
	  arImageList[arImageSrc[i]] = new Image();
	  arImageList[arImageSrc[i]].src = ICONPATH + arImageSrc[i];
...
function openReport(inURL)
{
	var windowWidth = window.screen.availwidth-100;
	var windowHeight = window.screen.availheight-100;
	//alert(inURL);
	//window.open(inURL+"", '_blank','width= 320px,height=450px,status=yes,toolbar=no,menubar=no,directories=no,resizable=no,location=no top=200,left=600' );
	//window.showModalDialog(inURL,null,"dialogHeight:450px;dialogWidth: 320px;resizable: No; status: No;help:no");
	window.showModalDialog(inURL,window,'dialogHeight:450px;dialogWidth: 320px;resizable: No; status: No;help:No;');
}
