...
MzTreeView.prototype.render = function()
{
  ...
  //隐藏根结点 lp
  //debugger;
  var id = me.index+"_tree_"+me.firstNode.index;
  if(str.indexOf("abcdroot")>-1){
   str="<div id='"+id+"' style='display: none;'></div>"; 
  }
  return "<div class='mztreeview' id='MTV_root_"+ this.index +"' "+
    "onclick='Instance(\""+ this.index +"\").clickHandle(event,"+this.checkflag+")' "+
    "ondblclick='Instance(\""+ this.index +"\").dblClickHandle(event)' "+
    "oncontextmenu='Instance(\""+ this.index +"\").contextMenuHandle(event)' "+ //lp 右键菜单
    ">"+ str +"</div>";
};
...
MzTreeView.prototype.getNode= function(id)
{
...
MzTreeView.prototype.clickHandle = function(e,checkflag) {
	 if(this.loading){ //add by jzy 节点加载过程不允许点击
		   alert("正在加载中，请稍候...");
		   return;
		 }
        ...
        case "IMG":
            if (B) {
                if (e.id.indexOf(this.index + "_expand_") == 0) {
					...
					var me = this;
					if(this.checkboxonclick!=null)setTimeout(function(){me.checkboxonclick(me)}, 10);
                }
            }
            break;
        case "A":
            if (B) { n.focus();
	               if(checkflag){
		               ...
		               n.expanded ? n.collapse() : n.expand(); this.dispatchEvent(new System.Event("onclick")); 
		               n.check(!n.checked,checkflag);
		               n.checkBox();
		               n.upCheck();
		                //选中checkbox展开 lp
		               if(!checkflag)n.expandAll(e.id);
	               }
            }
            break;
        default:
...
MzTreeView.prototype.contextMenuHandle = function(e)
{
  ...
  if(e.tagName=="A" && e.id)
  {
    ...
    //右键菜单 lp
   	if(e.$$caller.useCheckbox)createPopDiv(this);
  }
};
...
MzTreeView.prototype.dblClickHandle = function(e)
{
  ...
  if(e.tagName=="A" && e.id){
    ...
    e.focus();
    this.currentNode=e;this.dispatchEvent(new System.Event("ondblclick"));
    if(e.$$caller != undefined && !e.$$caller.useCheckbox)tree_labelDblclick(e);
  }
};
...
{
	var theMzTreeView = this;
	var tmpNodes=theMzTreeView.dataSource;
	var thelevel = level;
	window.setTimeout(function(){
		//新增判断如果展开节点数小于36个，即两屏的节点数，那么全部展开。by jiazhiyu @2012-02-09
		if(tmpNodes.length<36){
			level=10;
		}
		theMzTreeView.expandLeveling(level);
	},100);
};
MzTreeView.prototype.expandLeveling=function(level)
{
  ...
  //文本框中有值 单选树false 多选树true  lp
  /*
  if(r.$$caller.useCheckbox){
     searchMulNode(true,r.$$caller.checkflag);
  }else{
     searchMulNode(false);
  }
  */
...
t.childPrefix = "";
//private:get postion;
MzTreeView.prototype.getPosition = function()
{
   	  var $ = this.nodes;
   	  for(var m in $){
   	    if($[m].id=="root" || $[m].id=="-1")continue;
   	    if($[m].hasBox || $[m].$$caller.selectedNode){
			var y = 0;
			var Obj;
			if($[m].$$caller.selectedNode)
			Obj = document.getElementById($[m].$$caller.index +"_link_"+ $[m].$$caller.selectedNode.index);
			else
			Obj = document.getElementById($[m].$$caller.index +"_link_"+ $[m].index);
			
		  var pop = document.getElementById("pop_inner");
		  //弹出树定位
		  if(pop!=null){
		      for (var el = Obj; el; el = el.offsetParent) {
					y += el.offsetTop;
			  }
			  var stop =document.getElementById("pop_inner").scrollTop;
			  //alert(y+";"+stop);
			  if(y<stop){
			     document.getElementById("pop_inner").scrollTop = 0; 
			     document.getElementById("pop_inner").scrollTop = y-120;
			  }
			  else if(y>360){ //当前DIV高度360
			     document.getElementById("pop_inner").scrollTop = y-120;
		   	  }	  
	   	  }else{
	   	      //下拉树定位
	   	      ...
	   	      //alert(y+";"+divStop);
			  if(y>150){
			     tree_inner.childNodes[0].scrollTop = y-200;
		   	  }
	   	  }
	   	break;
	  }
...
MzTreeNode.prototype.loadChildNodes = function()
{
  ...
  if(this.$$caller.useCheckbox)
  {
    ...
    for(var i=0, n=this.childNodes.length; i<n; i++)
    {
      ...
      if(!node.parentNode.$$caller.checkflag)
      node.checked=node.parentNode.checked || node.checked;
    }
//if(n>0) this.childNodes[0].upCheck(); ///临时注掉。
  }
};
...
MzTreeNode.prototype.buildChildNodes = function()
{
  ...
  var nodeNum=mtv.dataSource.length;
  if(box = document.getElementById(mtv.index +"_tree_"+ me.index))
  {
    ...
    for(var i=0; i<=100 && i<nodecount; i++){
		var nhtml =me.childNodes[i].render(i==nodecount-1);
		var ndiv = document.createElement('div');
		nhtml = nhtml.substr(5,nhtml.length-11);
	...
	(function () {
		while(i<nodecount) {
			mtv.loading = true;
			nhtml = me.childNodes[i].render(i==nodecount-1);
			var ndiv = document.createElement('div');
			nhtml = nhtml.substr(5,nhtml.length-11);
			box.appendChild(ndiv);
			ndiv.innerHTML = nhtml;
		...
		if(i<nodecount){
			...
			var oChild=box.children(0);
			if(oChild.outerHTML.indexOf("load.gif")!=-1){
				box.removeChild(oChild);
			}
			r=mtv.rootNode;
			//文本框中有值 单选树false 多选树true  lp
			if(r.$$caller.useCheckbox){
			...
			if(nodeNum<36){	//弹出树定位判断 by jiazhiyu
			mtv.getPosition();
			}else{
				if(!mtv.loaded){
					mtv.getPosition();
	...
	if(me.checked) me.check(me.checked,me.$$caller.checkflag);
  }
};
//private: check checkbox
MzTreeNode.prototype.check = function(checked,checkflag)
{	 
...
// 中间节点选中 对子节点的hasBox属性进行初始化。by jiazhiyu 20120220
MzTreeNode.prototype.checkBox = function(){
	var n=this;
    if(n.hasChild){
        if(n.hasBox==0||n.hasBox==2){ //新增单击选中的时候对hasBox的修改，解决单击中间层节点，父级节点样式变化不正常的问题。 by jiazhiyu 2012-02-16
        	n.hasBox=1;
        }else if(n.hasBox==1){
        	n.hasBox=0;
        }
        ...
        for(var i=0;i<n.childNodes.length;i++){
        	n.childNodes[i].hasBox=tmp;
        	if(n.childNodes[i].hasChild){
        		for(var j=0;j<n.childNodes[i].childNodes.length;j++){
        			n.childNodes[i].childNodes[j].hasBox=tmp;
        		}
        	}
        }
    }else return;
...
/*
 * 修改upCheck方法，节点增加一个判断其图片的属性hasBox  值 0-未选中 1-对勾 2-点
 * @version 1.1 判断上层节点图片样式不再使用checked属性，而是使用新定义的hasBox属性
 * @author jiazhiyu 2012-02-06
 */
MzTreeNode.prototype.hasBox = 0;
MzTreeNode.prototype.upCheck = function()
{ 
  var node = this, mtv=node.$$caller, chk;
  if(!node.hasChild){
	  node.hasBox=node.checked?1:0;
  }
  if(node.parentNode){
  for(var i=0; i<node.parentNode.childNodes.length; i++)
  {
    if(node.parentNode.childNodes[i].hasBox != node.hasBox)
    {
    ...
    node = node.parentNode; if(!node.$$caller.checkflag){node.checked = this.checked;
    if (chk = document.getElementById(mtv.index +"_checkbox_"+ node.index)){
...
MzTreeNode.prototype.expand = function()
{
  ...
  {
    this.loadChildNodes();
    if(this.hasChild && (this.childNodes.length>100
      || /(^|\s|;)(JS|XML|UL)Data\s*:/i.test($.dataSource[this.sourceIndex])))
    {
      ...
      box.innerHTML="<table border='0' cellspacing='0' cellpadding='0'><tr><td>"+
        ...
        ics['load'].src +"'>"+ (!$.showNodeIcon ? "" : "<img border='0' src='"+
...
MzTreeNode.prototype.expandLevel = function(level) {
    ...
    if (level == 1) {
        if (this.hasChild) {
            ...
            if ($.lastNode != "" && this.childNodes[i].text == $.lastNode) $.isend = true;
        }
    }
...
MzTreeNode.prototype.collapse=function()
{
  ...
  if(line && box[this.line]!= undefined) line.src= box[this.line].src; this.expanded=false;
...
//cancel focus
MzTreeNode.prototype.cancelfocus=function()
{
  var $ = this.$$caller, a=$.rootNode, o;
  if(a = document.getElementById($.index +"_link_"+ this.index)){
  if(o = document.getElementById($.index +"_link_"+ $.selectedNode.index))
  o.className="";a.className="";
  $.dispatchEvent(new System.Event("cancelfocus"));}
