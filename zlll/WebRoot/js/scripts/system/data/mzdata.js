...
MzData.prototype.getNodeById = function(id)
{
  ...
  //----------------
  var _id = id;
  _id = _id.replace(/\(/g, "jackyxu").replace(/\)/g, "xujacky");
  var _indexes = this.indexes;
  _indexes = _indexes.replace(/\(/g, "jackyxu").replace(/\)/g, "xujacky");
  var reg=new RegExp("([^"+_+ d +"]+"+ d + _id +")("+_+"|$)");
  if (reg.test(_indexes)) { var s = RegExp.$1.replace(/(jackyxu)/g, "(").replace(/(xujacky)/g, ")");
  //----------------
...
MzDataNode.prototype.loadChildNodes = function(DataNodeClass)
{
  ...
  _indexes = _indexes.replace(/\(/g, "jackyxu").replace(/\)/g, "xujacky");
  var reg=new RegExp(_ + _id + r +"[^"+ _ + r +"]+", "g"); 
  if((cs=_indexes.match(reg))){for(i=0;i<cs.length;i++){
    tcn[tcn.length]=this.DTO(DataNodeClass, cs[i].substr(_.length).replace(/(jackyxu)/g, "(").replace(/(xujacky)/g, ")"));}}
  //----------------
  this.isLoaded = true;
};
